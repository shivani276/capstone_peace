...
def build_daily_incident_schedule(
    ...
    Priority is read from priority_col (default 'Final Priority') or defaults to 1.
    """
    tmp = df.copy()
...
def travel_time_minutes(lat1: float, lon1: float, lat2: float, lon2: float,kmph = np.clip(np.random.normal(40.0, 5.0), 20.0, 80.0)
) -> float:
